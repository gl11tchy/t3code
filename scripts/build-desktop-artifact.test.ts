...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    // GLITCHY: fork rebrand
    assert.equal(resolveDesktopProductName("0.0.17"), "Glitchy Code (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Glitchy Code (Nightly)");
  });
